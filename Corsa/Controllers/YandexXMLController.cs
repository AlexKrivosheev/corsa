...
{
    public class YandexXMLController : Controller
    {
        ...
        IStringLocalizer<YandexXMLController> _controllerLocalizer;
        IStringLocalizer<Module> _modulelocalizer;
        RuntimeContext _context;
        public YandexXMLController(IStringLocalizer<YandexXMLController> controllerLocalizer, IStringLocalizer<Module> moduleLocalizer, IServiceProvider provider, ISourceRepository repository, RuntimeTaskService taskManager, IProjectModuleRegistry modulRegistry, IConfiguration configuration, UserManager<IdentityUser> userManager, ILoggerFactory loggerFactory)
        {
            _provider = provider;
        ...
        {
            YandexXmlConfig config = new YandexXmlConfig() { ProjectModule = new ProjectModule() { ProjectId = project } };
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Create(YanexXMLModuleViewModel viewModel)
        {
            var modul = _modulRegistry.CreateModule<YandexXMLModule>(_context,YandexXMLModule.ModuleCode);
        ...
        [HttpPost]
        public IActionResult Edit(YanexXMLModuleViewModel viewModel)
        {
            var modul = _modulRegistry.OpenModule<YandexXMLModule>(_context, YandexXMLModule.ModuleCode, viewModel.Id);
            ...
            return View(new YanexXMLModuleViewModel(modul.Id, modul.Configuration) {
        ...
        {
            var modul = _modulRegistry.OpenModule<YandexXMLModule>(_context, YandexXMLModule.ModuleCode, id);
            return View(new YanexXMLModuleViewModel(id, modul.Configuration) {
                HttpModules = ProjectModuleRegistry.GetModules<HttpProviderRuntimeConfig, HttpProviderData>(_context, _modulRegistry, modul.Configuration.ProjectModule.Project.Id)
            });
        }
        ...
        {
            var module = _modulRegistry.OpenModule<YandexXMLModule>(_context, YandexXMLModule.ModuleCode, id);
            module.Drop();
